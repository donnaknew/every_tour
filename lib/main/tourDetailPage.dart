...
import 'package:every_tour/data/reviews.dart';
...
class _TourDetailPage extends State<TourDetailPage> {
  ...
  void initState() {
    ...
    widget.databaseReference!
        ...
        .child('review').onChildAdded.listen((event) {
              ...
              return Card(
                child: InkWell(
                  child: Padding(
                    ...
                    child: Text('${reviews[index].id} : ${reviews[index].review}', style: TextStyle(fontSize: 15),),
                  ),
                  onDoubleTap: () {
                            ...
                            return AlertDialog(
                              ...
                              actions: [
                                TextButton(
                                    onPressed: () {
                                      Review review =
                                      Review(widget.id!, _reviewTextController!.value.text, DateTime.now().toIso8601String());
                                      widget.databaseReference!
                                          .child('tour')
                                          .child(widget.tourData!.id.toString())
                                          .child('review')
        ...
        child: Column(
          children: [
            ...
            ElevatedButton(
              onPressed: () {
                DisableInfo info =
                DisableInfo(widget.id, disableCheck1.floor(), disableCheck2.floor(), DateTime.now().toIso8601String());
                widget.databaseReference!
                    .child('tour')
                    .child(widget.tourData!.id.toString())
                    .set(info.toJson())
